...
%SFPPB_BUILD 统一创建本文方法和参考控制器的 Simulink 模型。
%   SFPPB_BUILD("all") 创建全部三个模型；"main" 或 "comparison"
%   只创建对应模型。模型均采用 ode4、固定步长 0.001 s。
...
% Plant—Controller—Auxiliary—To Workspace，与 AGV_TFS 的模型分层一致。
global SFPPB_RL_P
...
add_block('simulink/User-Defined Functions/S-Function',[model '/SFPPB-RL controller'], ...
    'FunctionName','SFPPB_RL_ctrl','Position',[400 125 560 195]);
...
add_line(model,'Controller inputs/1','SFPPB-RL controller/1','autorouting','on');
add_line(model,'SFPPB-RL controller/1','Controller signals/1','autorouting','on');
