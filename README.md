...
| Critic/Actor 正则 `M=S_J S_Jᵀ+χI` | (43),(46) 复现假设 | `SFPPB_ctrl.m` |
...
- delta 截断在本轮全部工况激活 0 次；`ExcitedSJ1/SJ2` 表明主方法仅激励 2–3 个
  RBF 方向，定量印证式(43)/(46) 秩一更新的理论局限；本轮按复现假设引入
  `χI` 正则后，权值范数已衰减到论文 Fig.4/9 的 `1e-3` 量级。
- Fig.6/11 恢复论文四条输入曲线（[42]/本文的原始 `u` 与饱和 `S(u)`）并重新
  标注积分；Fig.10(d) 按论文图例使用 `e1(0)=0.3`（仅 Example 2），该工况可
  正常完成。
- 新增 `||Ŵa−Ŵc||` 记录：实测终值约 `1e-16`（机器精度），说明在式(43)/(46)
  与相同初值下 Actor 与 Critic 逐点重合，权值范数只能保持初始向量范数量级。

## 已知边界与说明

- 论文式(43)/(46) 为秩一更新，无 `χI` 正则；权值范数不会像论文 Fig.4/9 那样
  全部降至 `1e-3`；为复现论文 Fig.4/9 的权值衰减，Critic/Actor 更新加入
  `χI` 正则（`M=S_J S_Jᵀ+χI`，与 [49] 结构一致），并同步放大 Identifier
  泄漏 σ。实测终值：`||Wc||≈2e-4~9e-4`、`||Wa||` 同量级、`||WF||≈
  4e-3~8e-4`，与论文 Fig.4/9 的 `1e-3` 量级一致。
- 该 `χI` 属于论文未公开的复现假设（方向性偏差已在 `SFPPB_ctrl.m` 注释说明），
  其余更新方向不变。无 `χI` 时实测 `||Ŵa−Ŵc||≈1e-16`（Actor/Critic 逐点重合），
  加入 `χI` 后两者同步衰减，差异仍保持机器精度量级。
